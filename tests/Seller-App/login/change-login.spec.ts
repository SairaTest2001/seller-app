...
test('test', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Change Email/Mobile' }).click();
  await page.getByPlaceholder('Enter Email ID or Phone Number').dblclick();
  ...
  await page.getByRole('button', { name: 'Seller Admin' }).click();
});